...
def main(args):
    ...
    for hidden_dim in hidden_dim_options:
        for lr in lr_options:
            for history in history_options:
                ...
                if val_mae < best_by_T[history]["best_val"]:
                    best_by_T[history]["best_val"] = val_mae
                    best_by_T[history]["hidden_dim"] = hidden_dim
                    best_by_T[history]["lr"] = lr
                    best_by_T[history]["encoder_state"] = model.state_dict()
                    best_by_T[history]["regressor_state"] = downstream_gcn.state_dict()
    ...
    print("\n==============================")
    print("FINAL BENCHMARK RESULTS (per context window)")
    print("==============================")

    for history, info in best_by_T.items():
        hidden_dim = info["hidden_dim"]
        lr = info["lr"]

        print(f"\n--- Context window T={history} ---")
        print(f"Best hidden_dim={hidden_dim}, lr={lr}")
        ...
        model.load_state_dict(info["encoder_state"])
        ...
        downstream_gcn.load_state_dict(info["regressor_state"])
        downstream_gcn.eval()
        ...
        import pickle
        with open(f"records{history}.pkl", "wb") as f:
            pickle.dump(records, f)
        print(f"Saved records{history}.pkl for plotting.")
